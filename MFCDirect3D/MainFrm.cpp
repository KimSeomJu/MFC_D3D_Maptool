...
extern CMFCDirect3DView* g_pView;				// App 클래스의 View 포인트 클래스를 전역변수로 선언
// CMainFrame
...
// 화면을 나눌려면 OnCreateClient 함수 내에서 하는게 좋다
// CreateStatic : 행과 열을 만들어주는 함수
// CreateView : 행과 열로 나눈 분할된 화면을 보여줌
// App과 View 클래스를 연동시켜주는 View 포인트 클래스 초기화
	...
	CCreateContext* pContext)
{
	...
	g_pView = (CMFCDirect3DView *)m_wndSplitter.GetPane(0, 0);		// View 클래스 초기화
...
BOOL CMainFrame::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	cs.style &= ~FWS_ADDTOTITLE;		// 제목 타이틀에 "제목 없음"을 지워준다
	SetTitle(_TEXT("MFCDirect3D"));		// 타이틀 제목 설정
...
void CMainFrame::OnGetMinMaxInfo(MINMAXINFO* lpMMI)
{
	...
	lpMMI->ptMinTrackSize.x = CLIENTSIZE_X;
	lpMMI->ptMinTrackSize.y = CLIENTSIZE_Y;

	lpMMI->ptMaxTrackSize.x = CLIENTSIZE_X;
	lpMMI->ptMaxTrackSize.y = CLIENTSIZE_Y;
