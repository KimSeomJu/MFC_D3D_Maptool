...
namespace MyUtil{
	 ...
	 HRESULT CreateDevice(){
		 ...
		 if( d3d9 == NULL ){
			 MessageBoxA( g_hWnd, "Direct3DCreate9() - FAILED", "FAILED", MB_OK );
			 return E_FAIL;
		 }
	 ...
	 HRESULT CreateDevice(HWND ghwnd){
		 ...
		 //IDirect3D9 객체얻기가 실패하면 NULL 이 참조됨
		 if (d3d9 == NULL){
			 MessageBoxA(ghwnd, "Direct3DCreate9() - FAILED", "FAILED", MB_OK);
			 return E_FAIL;
		 }
		 ...
		 d3dpp.Flags = 0;						//일단 0
		 d3dpp.FullScreen_RefreshRateInHz = D3DPRESENT_RATE_DEFAULT;	//화면 주사율 ( 그냥 D3DPRESENT_RATE_DEFAULT 모니터 주사율과 동일시 )
		 d3dpp.hDeviceWindow = ghwnd;					//Device 가 출력될 윈도우 핸들
		 d3dpp.MultiSampleQuality = 0;						//멀티 샘플링 질 
		 d3dpp.MultiSampleType = D3DMULTISAMPLE_NONE;		//멀티 샘플링 타입 ( D3DMULTISAMPLE_NONE 안티알리아싱 사용안함 ) 
		 d3dpp.PresentationInterval = D3DPRESENT_INTERVAL_ONE;	//화면 전송 간격 ( 그냥 D3DPRESENT_INTERVAL_ONE 모니터 주사율과 동일시 )
		 d3dpp.SwapEffect = D3DSWAPEFFECT_DISCARD;	//화면 스왑 체인 방식 ( D3DSWAPEFFECT_DISCARD )
		 ...
		 if (FAILED(result))
		 {
			 ...
			 MessageBoxA(ghwnd, "CreateDevice() - FAILED", "FAILED", MB_OK);
			 return E_FAIL;
		 }
	...
	void SetMousePos(int x, int y, HWND ghwnd)
	{
		POINT setMousePos;
		setMousePos.x = x;
		setMousePos.y = y;
		...
		//Client 상의 위치를 Screen 영영역 범위로 바꾼다.
		ClientToScreen(ghwnd, &setMousePos);
