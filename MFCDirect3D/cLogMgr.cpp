...
VOID cLogMgr::LogFile(char* strmsg, ... )
{
	...
	if( m_nTarget & LOG_FILE )
	{
		wsprintfA(szBuf,"%s.txt",m_szFileName);
	}
	...
	if( fp == NULL )
	{
		MessageBoxA( 0, "에러 파일로 저장 실패", 0, 0 );
		return;	
	}
...
VOID cLogMgr::CreateLogWindow( HWND hWnd )
{
	...
	m_hWndListBox = CreateWindowA( "listbox", "", WS_CHILD | WS_VSCROLL, 0, 0, rc.right, rc.bottom, m_hWndLog, NULL, GetModuleHandle( NULL ), NULL );
