...
HRESULT cScene_00::Scene_Init()
{
	...
	cXMesh_Static* mesh = 
		...
		"../../MapToolResource/Resources/Meshes/Fighter/Fighter.X", &matCorrection );
	...
	this->pTrailRender->Init( 
		...
		RESOURCE_TEXTURE->GetResource( "../../MapToolResource/Resources/Testures/TrailTest.png" )	//외곡 그릴때 외곡 노말
		);
...
void cScene_00::Scene_Update( float timDelta )
{
	if( KEY_MGR->IsOnceDown( VK_RETURN ) ){
		...
		SCENE_MGR->ChangeSceneWithLoading("태진아", "로딩씬", 1, 1 );
